...
export function ProductsModal({
  ...
  const loadProducts = async () => {
    try {
      const response = await apiProduction.get(`/product/${shoppingListId}`);
  ...
  const handleDelete = async (productId: number) => {
    try {
      await apiProduction.delete(`/product/${productId}`);
      loadProducts();
    } catch (error) {
      handleRequestError(error as never);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      await apiProduction.put(`/product/${editingProduct.id}`, editingProduct);
      loadProducts();
